...
const CardPrice = ({ data = [], selected, onNext }) => {
        ...
        <Card key={index} className="mx-3 cardContainer" onClick={() => onSelected(item.id)}>
          <Card.Body className="cardBody justify-content-center m-0 p-0">
            <Card.Title className="cardTitle mx-auto">
              <span
                style={{ display: `${item?.special_text?.length !== 0 ? 'block' : 'none'}` }}
                className="bestSellTag"
              >
                ขายดี
              </span>
              {item.name}
            ...
            <div className="details-container">
              ...
              <span className="selectBtnCustom">เลือกแพ็คเกจนี้</span>
            </div>
          </Card.Body>
        </Card>
      )

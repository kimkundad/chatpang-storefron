...
const [isCheckAll, setIsCheckAll] =useState(false)
  ...
  const renderTable = () => {
    return data.map((item, index) => {
      return (
        <tr key={index}>
          <td><input type='checkbox' name={item.id} checked={itemList.includes(item.id)} onClick={(e)=>onChecked(e)} /></td>
          <td>{item.name}</td>
          <td>
          <div>
            ...
            <div className='userpage-wrapper text-center'>
              ...
              <div className='row'>
                <div className='col d-flex justify-content-center my-2'>
                  ...
                  <span onClick={onCopy} className='userButton'><FontAwesomeIcon className='me-2' icon={faCopy} />สร้างซ้ำ</span>
                  {/* <span className='userButton'><FontAwesomeIcon className='me-2' icon={faPenToSquare} />แก้ไข</span> */}
                  <span onClick={onDelete} className='userButton'><FontAwesomeIcon className='me-2' icon={faTrashCan} />ลบ</span>
                </div>
              </div>
              <div className='row'>
                <div className='col-md-8 mx-auto d-flex mt-3'>
                    <Table bordered >
                      <thead>
                        <th><input onChange={onCheckAll} type='checkbox' name='checkAll' checked={isCheckAll}/></th>
                        <th>แคมเปญ</th>
                        <th></th>
                      </thead>
                      <tbody>
